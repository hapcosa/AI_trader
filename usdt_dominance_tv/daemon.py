"""
USDT Dominance daemon — TradingView primary, CoinGecko fallback.

Polls TradingView CRYPTOCAP:USDT.D every minute, stores 1-minute OHLCV bars in
SQLite. On startup, backfills missing history. After consecutive TV failures,
falls back to CoinGecko until TV recovers.
...
def _initial_backfill(
    ...
    """If DB is empty or sparse, backfill HISTORY_DAYS from TV."""
    last_ts = storage.get_last_ts(conn)
    ...
    if last_ts is None:
        ...
        log.info("Empty DB — backfilling %d minutes (~%d days) from TV",
                 n_bars, n_bars // 1440)
    else:
        ...
        log.info("Gap of %d minutes detected at startup — fetching %d bars",
                 gap, n_bars)

    try:
        df = tv.fetch_recent(n_bars=n_bars)
        wrote = storage.upsert_bars(conn, df, source="tv")
        log.info("Backfilled %d bars from TV", wrote)
...
def _tick(
    ...
    try:
        ...
        log.info("TV ok — wrote=%d gap=%d last=%.6f", wrote, gap_minutes, last_close)
        state["tv_fails"] = 0
        return
    except Exception as e:
        state["tv_fails"] = state.get("tv_fails", 0) + 1
        log.warning("TV fetch failed (%d/%d): %s",
                    state["tv_fails"], fail_threshold, e)

    # Fallback to CoinGecko after threshold
    if state["tv_fails"] >= fail_threshold:
        value = cg_source.fetch_dominance()
        if value is not None:
            storage.upsert_single(conn, current_ts, value, source="coingecko")
            log.info("CoinGecko fallback wrote tick %s = %.4f%%",
                     now.strftime("%Y-%m-%d %H:%M UTC"), value)
        else:
...
def run() -> None:
    ...
    tv_symbol = _env_str("TV_SYMBOL", "USDT.D")
    ...
    log.info("Starting daemon — db=%s symbol=%s:%s poll=%ds history_days=%d auth=%s",
             db_path, tv_exchange, tv_symbol, poll_interval, history_days,
    ...
    try:
        tv = TVSource(
            symbol=tv_symbol, exchange=tv_exchange,
    ...
    if tv is not None:
        try:
            _initial_backfill(conn, tv, history_days)
        except Exception as e:
    ...
    try:
        while not stop.is_set():
            try:
                if tv is not None:
                    _tick(conn, tv, state, fail_threshold)
                else:
                    # Pure CoinGecko mode
                    now = datetime.now(tz=timezone.utc)
                    value = cg_source.fetch_dominance()
                    if value is not None:
                        storage.upsert_single(
                            conn, minute_ts(now), value, source="coingecko"
                        )
                        log.info("CG-only tick %.4f%%", value)
            except Exception as e:
